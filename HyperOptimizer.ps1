    ...
    foreach ($period in $PERIODS) {
        foreach ($risk in $RISK_LEVELS) {
            foreach ($dev in $DEV_LEVELS) {
                Write-Host "Running: $pair ($period) Risk=$risk Dev=$dev..." -ForegroundColor Yellow
                $cmdArgs = @("-ExecutionPolicy", "Bypass", "-File", $PS_SCRIPT, "-Pair", $pair, "-Risk", $risk, "-Dev", $dev, "-Period", $period)
                
                # Start the backtest script
                Start-Process -FilePath "powershell.exe" -ArgumentList $cmdArgs -Wait
                
                $report_file = Join-Path $RESULT_DIR "OptReport_$pair`_$period`_R$risk`_D$dev.html"
                ...
                if (Test-Path $report_file) {
                    ...
                    Write-Host "Failed to find report for $pair ($period) Risk=$risk Dev=$dev" -ForegroundColor Red
                }
            }
        }
    }
...
Write-Host "`n--- Optimization Complete (v4.20) ---" -ForegroundColor Cyan
